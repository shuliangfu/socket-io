...
| `server.ts:619` | `(socket as any).handleSocketIOPacket(data)` | 将 `handleSocketIOPacket` 改为 `protected` 或通过接口暴露 |
| `socketio/socket.ts:1130-1132` | `(this as any).id/nsp/handshake` | 使用 `Object.defineProperty` 或改为可写属性（对象池场景） |
...
| `emit(event, data?: any)` | 多处 | `data?: unknown` 或泛型 `emit<T>(event, data?: T)` |

---

## 二、日志与错误处理一致性

### 2.1 统一使用 logger 替代 console
...
| `adapters/mongodb.ts` | 292,541,674 | 适配器需接收 logger 或 onError 回调 |
| `adapters/redis.ts` | 479 | 同上 |
...
| `client/*` | 多处 | 客户端可保留 console（无 logger 依赖）或支持可选 logger |
| `compression-manager.ts` | 46,129 | 接收 logger 参数 |
| `heartbeat-manager.ts` | 98 | 接收 logger |
| `stream-parser.ts` | 214 | 接收 onError 回调 |
| `websocket-batch-sender.ts` | 66 | 接收 logger |
...
### 3.1 PollingTransport 暴露 hasPendingPackets
...
### 3.3 handleSocketIOPacket 可见性

- 方案 A：改为 `protected`，Server 通过子类或内部模块调用
- 方案 B：在 SocketIOSocket 上增加 `processPacket(data: string): void` 作为公开 API，内部调用 `handleSocketIOPacket`

---

## 四、适配器类型优化

### 4.1 MongoDB / Redis 适配器
...
适配器应支持 `onError` 或 `logger`，避免直接 `console.error`，便于上层统一处理。

---

## 五、与 @dreamer/websocket 的一致性

| 特性 | socket-io | websocket | 建议 |
|------|-----------|-----------|------|
| logger | ✅ 支持 | ✅ 支持 | 一致 |
| debug | ✅ 支持 | ✅ 支持 | 一致 |
| t 翻译 | ✅ 支持 | ✅ 支持 | 一致 |
| getServer() | ❌ 无 | ✅ 有 | socket-io 可增加 `socket.getServer()` 便于获取 Server 实例 |
| 错误输出 | 部分 console | 部分 logger | 统一为 logger |
...
- 将 `id`、`nsp`、`handshake` 改为可写（非 readonly），仅在对象池场景下由内部写入
...
| 中 | 统一 logger 替代 console | 中 | 一致性、可配置 |
...
socket-io 功能完整、测试充分（151 个用例），主要优化方向为：

1. **类型安全**：消除 `as any`，用正式 API 或方法替代
2. **日志一致性**：子模块统一使用 logger，避免直接 console
3. **类型细化**：将 `any` 收窄为 `unknown` 或具体类型
4. **与 websocket 对齐**：如 `getServer()` 等便捷 API

建议按优先级分批实施，优先完成「高」优先级项，再逐步推进「中」「低」优先级优化。
