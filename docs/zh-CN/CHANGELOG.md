...
- **`src/engine/websocket-batch-sender.ts`**：`ws.send(task.data)` 加 cast 为
  ...
  （运行时值不变）。
- **`publish.yml`**：仅 tags 触发（已为 `npx jsr publish`，无 `--no-check`）。
...
- **加密**：客户端与服务端加密管理器改为从 `@dreamer/crypto/client` 引入
  `decrypt`/`encrypt`，不再从 `@dreamer/crypto` 引入，避免客户端打包带入
  i18n/locales 导致浏览器解析错误。
